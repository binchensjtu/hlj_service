...
for ($i = count($trades)-1; $i >= 0; $i--) {
    ...
    if ($tidRecord) {
        ...
        if($tidRecord != $trades[$i]['status']) {
            // 判断是否需要更新交易表格
            // if($trades[$i]['status'] == )
            if($trades[$i]['status'] != "TRADE_NO_CREATE_PAY") {
        ...
        if($a) {
            ...
            if($logi) {
                ...
                if($trades[$i]['status'] == "TRADE_NO_CREATE_PAY") {
                    $para = array("wechat_id"=>$trades[$i]['weixin_user_id'],"country"=>$a[0]['country'],"item_info"=>$a[0]['title'],
                        "count"=>$trades[$i]['num'],"ac_time"=>$ac_time,"logistic_id"=>$logi,"pub_id"=>$a[0]['id'],
            ...
            if($pre) {
                if($pre[0]['is_buyed'] == 1) {
                    ...
                    if($logi) {
                        ...
                        if($trades[$i]['status'] == "TRADE_NO_CREATE_PAY") {
                            $para = array("wechat_id"=>$uwid,"country"=>$pre[0]['country'],"item_info"=>$pre[0]['title'],
                                "count"=>$trades[$i]['num'],"ac_time"=>$ac_time,"logistic_id"=>$logi,
                    ...
                    if($trades[$i]['status'] == "TRADE_NO_CREATE_PAY") {
                        $paraInner = array(
                            "count"=>$trades[$i]['num'],"ac_time"=>$ac_time,
