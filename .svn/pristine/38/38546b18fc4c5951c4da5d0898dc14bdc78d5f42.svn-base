...
function getDHL() {
    echo "DHL_GERMAN\n";
    $url = 'http://nolp.dhl.de/nextt-online-public/set_identcodes.do?lang=en&idc=540361122385';
    $html2 = new simple_html_dom();
    $html2->load_file($url);
    foreach($html2->find('.accordion-inner table tr') as $element) {
        $temp = $element->find('td text');
        if(count($temp) == 3) {
            echo trim($temp[0]) ."    " . trim($temp[1]) . "    " . trim($temp[2])."\n";
        }
    }
